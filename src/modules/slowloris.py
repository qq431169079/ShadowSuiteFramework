...
def module_body():
    ...
    SOCKETS = input("Number of sockets to use in the test > ")
    ASK_USEPROXY = input("Do you want to use a SOCKS5 proxy for connecting? (y/n) > ")
    if ASK_USEPROXY == 'y':
        USEPROXY_SWITCH = "-x "
        USEPROXY_HSW = "--proxy-host "
    ...
    print("[i] Running test in 3...")
    time.sleep(1)
    print("[i] Running test in 2...")
    time.sleep(1)
